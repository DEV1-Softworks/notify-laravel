...
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) (newest entries first),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.2.1] — 2026-04-24

**Added**
- PHP **8.5** is now part of the CI matrix alongside 8.4. Composer's existing `^7.4|^8.0` constraint already permitted 8.5; this release validates it in CI and documents the supported range.

**Fixed**
- `release.yml` was extracting the **oldest** version from `CHANGELOG.md` (top-of-file) instead of the newest, which caused the 1.2.0 merge to attempt a `v1.0.0` tag. The CHANGELOG is now ordered newest-first per the Keep a Changelog convention, and the workflow's `head -n1` regex now resolves to the current release correctly.

**Docs / Tests**
- Test coverage raised from ~45% to ~98% via new tests exercising the `fcm_v1` driver registration path (inline JSON, file path, cache-store wiring, all error branches), the anonymous `Notifier` payload merge (value objects, `platform_defaults`, empty-string target normalization), `NotifyChannel` early-return branches, and `LaravelLogger`'s PSR/`getLogger()`/closure-fallback branches.
- README updated to reflect **PHP 7.4 – 8.5** support.

**Backward compatibility**
- No public API changes. No config changes. Drop-in upgrade from 1.2.0.

---

## [1.2.0] — 2026-04-18

Bumps the floor of `dev1/notify-core` to **^1.2**, aligns the adapter with core 1.2.0's new contracts, exposes the new retry/caching knobs, and widens framework compatibility to Laravel 8 – 13.
